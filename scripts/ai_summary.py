...
def get_as_of_date(cur):
    """Latest date with price data. Crypto runs ahead of stocks, so this is
    the crypto date -- stock rows are pulled at their own latest date."""
    cur.execute("select max(date) from public_intermediate.asset_returns")
    return cur.fetchone()[0]


def get_latest_prices(cur):
    """Most recent row per asset. Stocks do not trade on weekends, so their
    latest date can lag the crypto one by up to three days."""
    cur.execute("""
        select symbol, asset_type, price, daily_return_pct, date
        from (
            select
                symbol,
                asset_type,
            ...
            from public_intermediate.asset_returns
        ) ranked
        where rn = 1
...
def get_correlations(cur):
    """Latest 30d correlation per pair, averaged by pair type. Taking the
    latest row per pair (rather than the latest date overall) keeps stock
    pairs in the picture even though they lag behind crypto."""
    cur.execute("""
        with latest as (
            select
                pair_type,
                correlation_30d,
                row_number() over (
                    partition by symbol_a, symbol_b order by date_key desc
                ) as rn
            from public_marts.fact_asset_correlation
            where correlation_30d is not null
        )
        select
            pair_type,
            round(avg(correlation_30d), 2) as avg_correlation,
            count(*) as pair_count
        from latest
        where rn = 1
        group by pair_type
...
def gather_snapshot(cur):
    ...
    if as_of_date is None:
        raise RuntimeError("asset_returns is empty -- run dbt before this task.")
