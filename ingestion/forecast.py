...
def load_panel(conn):
    """All completed daily candles for the five crypto assets (Binance OHLCV,
    joined onto CoinGecko dates in int_crypto_prices and pre-aggregated in
    int_binance_metrics).
...
def run():
    ...
    if panel.empty:
        raise RuntimeError("int_binance_metrics is empty -- run dbt before this task.")
